...
folder_map = {
    "Shopping da Ilha": r"C:\Program Files\Victor & Schellenberger\VSSC_ILHA_HOM",
    "Shopping Mestre Álvaro": r"C:\Program Files\Victor & Schellenberger\VSSC_MESTREALVARO_HOM",
    "Shopping Metrópole": r"C:\Program Files\Victor & Schellenberger\VSSC_METROPOLE_HOM",
    "Shopping Montserrat": r"C:\Program Files\Victor & Schellenberger\VSSC_MONTSERRAT_HOM",
    "Shopping Moxuara": r"C:\Program Files\Victor & Schellenberger\VSSC_MOXUARA_HOM",
    "Shopping Praia da Costa": r"C:\Program Files\Victor & Schellenberger\VSSC_PRAIADACOSTA_HOM",
    "Shopping Rio Poty": r"C:\Program Files\Victor & Schellenberger\VSSC_TERESINA_HOM"
}
...
folder = folder_map.get(
    shopping_escolhido,
    r"C:\Program Files\Victor & Schellenberger_FAT_HOM\VSSC_MONTSERRAT_HOM"
...
def analyze_screenshot_login(image_path):
    """
    Converte a imagem para Base64 e envia para a API do GPT-4 Turbo (com visão),
    retornando todo o texto extraído da tela.
    """
    if not os.path.exists(image_path):
        logging.error("Nenhuma imagem válida encontrada para análise.")
        return None
    ...
    try:
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": (
                                "Identifique a tela em foco no print, normalmente um modal ou um alerta, "
        ...
        extracted_text = response.choices[0].message.content.strip()
        logging.info("Texto retornado pelo GPT-4 Turbo: %s", extracted_text)
        return extracted_text

    except Exception as e:
        # Se for erro de base64, ignora — senão, registra
        if "base64" not in str(e).lower():
            logging.error("Erro ao analisar imagem: %s", e)
        return None
