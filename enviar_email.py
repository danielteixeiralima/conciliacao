...
from utils import login, gerar_competencia, folder_map
import unicodedata
import re
...
def normalize_text(s):
    ...
    s = re.sub(r'[^a-z0-9\s]', ' ', s)   # remove símbolos tipo "-" ou "."
        ...
        for fase in fases:
            ...
            while True:
                ...
                if fuzzy_contains(combined_extracted, "Alerta VSSC") and fuzzy_contains(combined_extracted, "A área de recibo já foi gerada"):
                    ...
                    time.sleep(10)
                elif fuzzy_contains(combined_extracted, "Lista de Recibos Já Emitidos") and not fuzzy_contains(combined_extracted, "Verificando"):
                    ...
                    logging.info("execute_vsloader: Alerta VSSC. Print indica ação ESC. Texto identificado: %s", combined_extracted) 
                    pyautogui.moveTo(center_x, center_y)
                    pyautogui.click()
                    time.sleep(0.5)
                    pyautogui.press("esc")
                ...
                if fuzzy_contains(combined_extracted, "Alerta VSSC") and fuzzy_contains(combined_extracted, "Não há emissão"):
                    ...
                    while True:
                        ...
                        if fuzzy_contains(combined_extracted, "Alerta VSSC") and fuzzy_contains(combined_extracted, "A área de recibo já foi gerada"):
                            ...
                            pyautogui.click()
                            pyautogui.press("enter")
                        elif fuzzy_contains(combined_extracted, "Enviando e-mail"):
                            logging.info("execute_vsloader: Enviando e-mail. Texto identificado: %s", combined_extracted)
                            time.sleep(10)
                        
                    
                        elif fuzzy_contains(combined_extracted, "Envio de boletos por e-mail concluído"):
                            logging.info("execute_vsloader: Processando código. Texto identificado: %s", combined_extracted)
                            ...
                            for row in ws.iter_rows(min_row=3):
                                ...
                                if row[base].value == fase:
                                    row[base+2].value = "Enviado!"
                                    updated = True
                                    logging.info("Atualizado envio de e‑mail no Excel para fase %s: %s", fase, row[base+2].value)
                                    break
                            if not updated:
                                logging.warning("Nenhuma linha encontrada no Excel para atualizar para fase %s", fase)
                            wb.save(excel_filename)
...
if __name__ == "__main__":
    ...
    log_filename = os.path.join(log_dir, f"Enviar_email_{shopping_abreviado}_{tipo_escolhido}.txt")
